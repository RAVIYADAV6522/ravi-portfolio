          ...
          {projects.map((project, idx) => (
            ...
            >
              ...
              <div className="flex flex-grow flex-col p-6 sm:p-8">
                <h3 className="text-xl font-black tracking-tight text-zinc-900 group-hover:text-zinc-700 sm:text-2xl">
                  {project.title}
                </h3>
                <p className="mt-4 flex-grow text-base leading-relaxed text-zinc-600 line-clamp-3">
                  {project.description}
                </p>

                {/* Tech Tags - Reduced Margin */}
                <div className="mt-8 flex flex-wrap gap-2">
                  {project.tags.map((tag) => (
                    <span
                      key={tag}
                      className="rounded-lg border border-zinc-100 bg-zinc-50 px-3 py-1 text-[0.7rem] font-bold text-zinc-500 transition-colors group-hover:border-zinc-200 group-hover:bg-zinc-100 group-hover:text-zinc-900"
                    >
                      {tag}
                    </span>
                  ))}
